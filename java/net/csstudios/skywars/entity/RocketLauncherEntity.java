...
import net.csstudios.skywars.procedures.RocketParticleProcedure;
import net.csstudios.skywars.procedures.RocketExplodeProcedure;
...
public class RocketLauncherEntity extends AbstractArrow implements ItemSupplier {
	public RocketLauncherEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(CsSkywarsModEntities.ROCKET_LAUNCHER.get(), world);
	}
	...
	public ItemStack getItem() {
		return new ItemStack(CsSkywarsModBlocks.CORPUS_STEEL_ENERGY_CONTAINER.get());
	}

	@Override
	protected ItemStack getPickupItem() {
		return new ItemStack(CsSkywarsModItems.ROCKET_LAUNCHER.get());
	}
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		RocketExplodeProcedure.execute(this.level, this.getX(), this.getY(), this.getZ());
	}

	@Override
	public void onHitBlock(BlockHitResult blockHitResult) {
		super.onHitBlock(blockHitResult);
		RocketExplodeProcedure.execute(this.level, blockHitResult.getBlockPos().getX(), blockHitResult.getBlockPos().getY(), blockHitResult.getBlockPos().getZ());
	}

	@Override
	public void tick() {
		super.tick();
		RocketParticleProcedure.execute(this.level, this.getX(), this.getY(), this.getZ());
		if (this.inGround)
			this.discard();
	}
	...
	public static RocketLauncherEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		RocketLauncherEntity entityarrow = new RocketLauncherEntity(CsSkywarsModEntities.ROCKET_LAUNCHER.get(), entity, world);
	...
	public static RocketLauncherEntity shoot(LivingEntity entity, LivingEntity target) {
		RocketLauncherEntity entityarrow = new RocketLauncherEntity(CsSkywarsModEntities.ROCKET_LAUNCHER.get(), entity, entity.level);
